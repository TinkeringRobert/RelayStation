...
module.exports = {
  ...
  getEnergyMeterValueDay: function(datetime, callback) {
    if(db === null || db_initalized === false){
      winston.error("Database call not handled db:" + db + " init:" + (db_initalized===true ? "true" : "false"));
      return callback(undefined);
    }

    var startDatetime = moment(datetime).startOf("day").format('YYYY-MM-DD');
    var endDatetime = moment(datetime).startOf("day").add(1, "day").format('YYYY-MM-DD');

    var query = "SELECT * FROM energy_meter WHERE timestamp >= ? AND timestamp < ? ORDER BY timestamp DESC";
    db.serialize(function() {
      db.all(query, startDatetime, endDatetime, function(err, rows) {
        if( err !== null)
        {
          winston.error("ERR: " + err);
          callback(undefined);
